...
**差分审计基线**：`converg-baseline.json`（`chemkit.converg.dump()` 产出）是当前口径的逐例权威快照——`digest_all = 088a6a368c125b0c`、n=1173、iters_total=12043、resid_p90 0.033、resid_max 8.073（`n(|S|>1)` 59）。动判定语义前先跑 `diff()` 逐例对照，**但位移只是辅助参考、不是门槛**：门槛是断言（化学），位移只用来复核"有没有变坏"。实测标定了这件事的量级——二分提前 1e-11 收敛会翻转 42 例走步路径（多为收敛变好），而修完"锁轨迹"的 3 条标准后**断言 0 例翻红**，于是这块性能照常落地（见 architecture §7 V/W）。
