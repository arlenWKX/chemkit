...
**差分审计基线**：`converg-baseline.json`（`chemkit.converg.dump()` 产出）是当前口径的逐例权威快照——`digest_all = 0785ec3b515daca0`、n=1173、iters_total=11905、resid_p90 0.033。动判定语义前先跑 `diff()` 逐例对照，再决定是否接受位移（实测：连"二分提前 1e-11 收敛"这种量级都会翻转 42 例，见 architecture §7 V）。
...
最近一次全量（1173 例 / 1294 断言，Windows 沙箱，单遍）：
...
- 均值 **30.8 ms**；中位 16.1 ms；P90 68.4 ms；P99 257 ms；最值 495 ms；
  逐例合计 36.2 s（整轮墙钟 ~40 s，含预热与环闭合检查）。
- **时间高度集中**：≤10 ms 的 294 例只占 4.4%；100–500 ms 的 75 例独占 36.2%。
- 最慢一档：Hg2_1（240 步）、Ni41（159 步）、D32（104 步）、P11（94 步）、
  N06、X10、N30、TS04。**慢例 = 步数多 × 每步成本**：剖面显示每步的钱主要
  花在 `solve_extent` 的二分探针（`S_of` 33% + pH 机器 20%，每次 f 各一遍）
  ——"减少步数"的四条路已实测证伪（§7 M-2 / T-2 / T-3 / U-2），下一轮的
  杠杆是**保括号的加速求根**（Illinois/Brent：保括号 ⟹ 同根，~47 → ~8–12 次）。
- 结构性事实：**22.3–22.7% 的用例带“反复逼近”签名**（同一净反应重复执行
  ≥3 次，见 `tools/osc.py`）。其中 **84% 出口残差 live ≤ 0.05**——它们
  收敛了、只是慢（几何级数的代价），另 16% 是欠收敛/数据张力。
...
2. **走步的反复逼近**：22.3–22.7% 用例带重复签名；终态对，但收敛靠几何
   级数，精度上限 ~ρⁿ（N15 的 `1.999CO_2` 即此残差的呈现）。其中 **84%
   只是慢**（可外推加速），16% 是欠收敛/数据张力。**“把 walk 换成联立求解”
   已被两次实测证伪**（§7 M-2 循环键子集、§7 T-2 整个活性社区：后者让 E35
   从 11 步涨到 3010 步）——语义内的外推亦已实测证伪（§7 T-3）。
