...
| `H39 CO2 过量+NaOH` | 过量 CO₂ 逸出与 HCO₃⁻ 配比的呈现口径 |
| `H89 Ca(HCO3)2+Ca(OH)2` | 分步方程缺一条（通道合并顺序） |
| `P10 Fe(OH)3 不溶 NH4Cl` | 净方程退化（反应级联的呈现） |
...
3. **pH 机器是教科书近似**：Ksp / 气相储库决定的 pH（Cu(OH)₂ 饱和液、
   浓碱溶铝等）由启发式分支给出，`charge_pH`（精确质子条件）在这些体系
   上给出的反而是"把账本拉回自洽"的假根——两者各有适用域，替换判据见
   architecture §7 F。
