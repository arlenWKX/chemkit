...
cd chemkit
python -m chemkit.testsuit # 1245/1247 断言 + 热力学环闭合检查（~40 s）
```

> **基线如实声明（0.4.3 审计）**：当前 HEAD 实测 **1245/1247**，两条
> 长期红例为 **V16**（SO₂+CaCl₂ 溶液 仅溶出不沉淀）与 **Z31**
> （Na₂SiO₃+半量盐酸 部分析胶）——两者都源于 pH 机器的酸碱近似，
> 详见 architecture.md §7「0.4.x 现状审计」。仓库内的
> `converg-baseline.json` 是更早工作树状态的快照（digest 与 HEAD
> 不符），差分审计前需重采。
