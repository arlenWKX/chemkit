...
- **施工顺序（钉死）**：①He 拆源与守恒修复 → ②重采 converg 基线 →
  ③以 acidbase 精确分布替换 pH 机器分支 4 与 1e-3 端点（J06 酸侧
  悬崖、Z31 硅酸梯截断、TS04 残差张力的共同出口）→ ④联立求解器
  （log 域 Newton + Gram-Schmidt 秩归约，H46 秩亏的正解）重新接入。
  本版已完成 ④ 的数学骨架与 ③ 的求解核，但均被 ① 拦在验收线外。
  见 architecture.md §7「0.4.x 现状审计」。
