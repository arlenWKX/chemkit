...
> **基线如实声明**：当前 HEAD 实测 **1294/1294** 断言通过（全绿；
> 全绿只等于"已知问题都被解释过"，不等于没问题）。任何改动都必须跑全量复核——
...
用例断言维度：`changed` / `reacted` / `degree`（int 或 str）/ `has` / `has_not` /
`has_range` / `has_any` / `has_initial` / `has_in_initial_not` / `ph` / `ann` /
`override` / `eq`（净方程）/ `eq_has`（分步方程**须被叙述覆盖**，见下）。
...
**`eq_has` 是"张成"判据而非"逐字出现"判据**（v0.5.0）：把各步与需求都写成
净变换向量（精确有理数），只要存在**非负系数**线性组合把需求**张成**就算覆盖
（`testsuit._step_spanned`）。理由：同一条净变换可以经由不同中间体合法到达
（PbSO₄ 既可走 `[Pb(OH)₃]⁻` 也可走 Pb(OH)₂/Pb²⁺；Al(OH)₄⁻ 既可经碳酸盐中间体
也可直接碳酸化），要求逐字出现等于把**路线**当化学。非负系数才是真约束——
负系数意味着要"倒着走"某一步，那不是合法叙述；少原子、电荷不平、需要倒走
照样判错。规范方向 `H⁺+OH⁻→H₂O`（呈现规范，§7 O-5）作为自由列参与张成。

**断言必须锁化学、不能锁轨迹**（v0.5.0 纪律）：判据是 `tools/fragility.py`——
把投料做 `(1±1e-11)`／`(1±1e-9)` 相对扰动（1 mol 只差 1 nmol，物理上无意义）
重跑全库断言，**仍翻红的断言就是对数值噪声敏感、锁了轨迹**，必须改写成
区间/下限/守恒式。当前实测 **0 例翻红**（修 P11/N19/Z31 前是 2 例）。

**当前基线**：**1294/1294 PASS（全绿）**（1294 = 1202 用例断言 + 92 条酸碱地基断言）。这些数字是
`python -m chemkit.testsuit` 自报的口径，任何改动都必须跑全量复核。

**当前无长期红例**。绿是逐条裁决换来的：标准守恒审查（`tools/eqcheck.py`
全库 0 违规）、"引擎 vs 标准"的状态自洽裁决、叙述规范形、显著度口径统一、
选相决策（详见 architecture.md §7 O–S）。改动任何判定语义都必须重跑全量 +
`tools/phcmp.py` / `tools/phdiag.py` 的对照审计。

**差分审计基线**：`converg-baseline.json`（`chemkit.converg.dump()` 产出）是当前口径的逐例权威快照——`digest_all = 213a116113124199`、n=1173、iters_total=12144、sof_total=844991、resid_p90 0.008、resid_max 8.073（`n(|S|>1)` 35）。**残差口径（v0.5.0 修正）**：S 一律在**求解器自己的 pH** 上评（`pH_solver`＝closed_pH 快路径／estimate_pH 用原始 He），呈现 pH 上的值单列 `S_pres`——旧口径用呈现 pH 评残差，等于换一个态去质问求解器（B3 档呈现 pH 是精确质子条件、求解器 pH 是启发式，两者可差几个单位），把假象算进张力例（`n(|S|>1)` 59→35、p90 0.024→0.008）。动判定语义前先跑 `diff()` 逐例对照，**但位移只是辅助参考、不是门槛**：门槛是断言（化学）。实测标定了这件事的量级——二分提前 1e-11 收敛会翻转 42 例走步路径（多为收敛变好），而修完"锁轨迹"的 3 条标准后**断言 0 例翻红**，于是这块性能照常落地（见 architecture §7 V/W）。同一把尺子也用在"两侧同态"这类根因修复上：§7 X-9 的改动让逐例 digest 全变，但断言 1294/1294、脆弱性审计 0 翻红、残差 p90 反而 0.033→0.024。


## 性能

**口径先说清楚（v0.5.0 纪律）**：**裁决性能改动用确定性计数，不用墙钟。**
共享沙箱里墙钟的第一遍惩罚高达 27%——同一份代码、同一进程内跑三遍全量：
53.6 s（首遍）/ 40.3 s / 42.1 s（`tools/sof_ab.py`，其中首末两遍是**同一份
旧代码**，`iters`/`steps`/逐例 digest 完全一致）。所以跨时间比较均值（"30.8
还是 33.9 ms"）没有意义，只能做**同进程/同机相邻对照**。硬指标是
`S_of` 调用次数（`engine.SOF_CALLS`，同一份代码 ⇒ 同一计数）：

```
off（跑满浮点饱和） → on（收敛容差 1e-11）
S_of 调用 1116659 → 843994   −24.4%  ← 确定性净额（已含走步变长的代价）
iters       11905 → 12043    +1.2%   steps 9500 → 9637  +1.4%
同进程暖态墙钟 42.1 s → 40.3 s  −4.3%（冷启首遍 53.6 s，不可比）
```

**逐例判读**同样不要用收敛 dump 的 `ms`（含 GC 停顿，单例可被放大数千倍），
要用 `iters` / `steps` / `sof`（dump 摘要新增 `sof_total`）。

**下一块性能（~2×）已定位但被判否**（§7 X）：全量 `S_of` 调用约六成花在主求解
二分上（17381 个括号 × ~31 次求值），保括号假位法（Illinois）只需 ~15 次
（实测 **2.04×**）——但审计发现 **2.6% 的括号里 f 有"−/+ 口袋"**（近似 pH 机器
切换分支造出的伪结构）：二分返回**最左根 = 反应第一次 S=0 的停止点**，而
Illinois 会跳进口袋上方的正区、收敛到**第二个**穿过点 ⟹ **过执行**
（T50 型 0.016→0.027）。这不是"测试被锁"，是求根法本身错，故判否。
把 f 与 pH(x) 对齐后**证实口袋就是 pH 跳变造的**：pH 机器换分支时 pH 在
一个网格步内跳 3.7–6.3 个单位（化学上 pH(x) 该是连续的滴定曲线）。
全库普查：**23.2% 的求根括号里 pH(x) 相邻点跳变 >1.0 单位、20.4% >3.0
（最大 9.10）**，集中的全是两性/固相族（Al/Ga/Sb 的 ⇌[M(OH)₄]⁻、Cu(OH)₂…）。
这就是下一步的主攻方向：让 pH 由**一套平衡条件**决定，见 §7 X-3/X-4。

最近一次全量（1173 例 / 1294 断言，Windows 沙箱，冷启单遍，均值 33.0 ms；
同一台机器同一周内同代码的冷启单遍曾在 30.8 ms——**差异是机器负载，不是代码**）：
...
- 均值 **33.0 ms**；中位 17.7 ms；P90 70.2 ms；最值 558 ms；
  逐例合计 38.7 s（整轮墙钟 ~40 s，含预热与环闭合检查）。
- **时间高度集中**：≤10 ms 的 273 例（23.3%）只花掉零头时间；
  100–500 ms 的 77 例（6.6%）吃掉约三分之一。
- 最慢一档（单例 ms 降序）：Hg2_1、P11、D32、Ni41、X10、X04、N30、Cu31；
  步数最多的是 Hg2_1（240 步、单净反应最多重复 37 次）、RX13（160 步、
  重复 45 次）、Ni41（159 步）、Cu31（148 步，`tools/osc.py`）。
  **慢例 = 步数多 × 每步成本**：剖面显示每步的钱主要
  花在 `solve_extent` 的二分探针（`S_of` 33% + pH 机器 20%，每次 f 各一遍）
  ——"减少步数"的四条路已实测证伪（§7 M-2 / T-2 / T-3 / U-2），
  **每步成本**这块已落地第一刀（二分收敛容差，`S_of` 调用 −24.4%，§7 W-4/W-5）。
- 结构性事实：**23.0% 的用例带“反复逼近”签名**（同一净反应重复执行
  ≥3 次，见 `tools/osc.py`：270/1173，总步数 9622、最差 240 步）。其中
  **85% 出口残差 live ≤ 0.05**——它们收敛了、只是慢（几何级数的代价），
  另 15%（40 例）是欠收敛/数据张力。
  “把 walk 换成联立求解”已被两次实测证伪（循环键子集 §7 M-2、整个活性
  社区 §7 T-2：后者让 E35 从 11 步涨到 3010 步）；**语义内的外推
  也已实测证伪**（§7 T-3：慢例是螺旋不是直线，一维几何外推的
  有效域就是微步档）。
...
1. **测试基线**：当前 **1294/1294 全绿**（用例断言 1202 + 酸碱地基断言 92）。
   绿是逐条裁决换来的，不等于没问题——改动判定语义必须重跑全量 +
   `tools/eqcheck.py` / `tools/osc.py` / `tools/phdiag.py` 的对照审计。
2. **走步的反复逼近**：23.0% 用例带重复签名；终态对，但收敛靠几何
   级数，精度上限 ~ρⁿ（N15 的 `1.999CO_2` 即此残差的呈现）。其中 **85%
   只是慢**（可外推加速），15%（40 例）是欠收敛/数据张力。**“把 walk 换成
   联立求解”已被两次实测证伪**（§7 M-2 循环键子集、§7 T-2 整个活性社区：
   后者让 E35 从 11 步涨到 3010 步）——语义内的外推亦已实测证伪（§7 T-3）。
3. **pH 机器是教科书近似**：走步内层（二分探针）用四分支启发式——Ksp /
   气相储库决定的 pH（Cu(OH)₂ 饱和液、浓碱溶铝等）由启发式给出，
   `charge_pH`（精确质子条件）在这些体系上给出的反而是“把账本拉回自洽”
   的假根。**呈现层已按 B3 判据切到精确解**（`exact_proton_pH`：账本电中性
   自洽 ∧ 无固相/水解阳离子储库 ∧ 有可再分配质子化族），报告的 `final_pH`
   在该档是精确值（B3 档与精确解的偏差例数 25 → 0）；判据与实验见 §7 N/O。
   **代价不止精度**：分支切换让 pH(x) 不连续 ⟹ S(x) 长出"−/+ 口袋"
   （实测 2.6% 的求根括号，§7 X），于是更快的求根法（2.04×）不能用——
   近似把"pH 对组成连续"这条本该成立的性质破坏掉了。
4. **固相选相**：`phase` 只做声明，221 处固相析出量 <20% 投料的体系对
   ±0.5 个 pKsp 敏感（`tools/hydrate_audit.py` 的 ⚠ 清单）；换相是化学决策，
   须有出处、反证与影响面核查（§7 S 的 CaSO₃ 即一例）。
