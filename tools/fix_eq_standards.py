...
REASON = {
    "Q05 H2S过量+NaOH":
        "【v0.5.0 标准修正·叙述化石】原 eq 为「H₂S + OH⁻ → HS⁻ + H₂O」——"
        "它是**旧 pH 10.50 的化石**：H₂S 1 + NaOH 1（半中和）的精确 pH 为 "
        "**7.00 = pKa₁**（1:1 H₂S/HS⁻ 缓冲对，charge_pH 与教科书两性/缓冲"
        "公式一致），不再落在引擎叙述政策（终态 pH > 7 用 OH⁻、否则用 H⁺）"
        "的碱性侧，故与同情境的 B12/H78（醋酸半中和，pH 4.76）一样写作"
        "解离式；同一化学而 NaOH 更少的 AB03（pH 9.84）仍保留 OH⁻ 式。"
        "叙述政策本身是待定项，见 architecture §7 O-5。",
}
