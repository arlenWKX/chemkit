...
高层 API（两个对外对象）：
    import chemkit
    sys = chemkit.System(V=1.0, T=298.15, p=101.3)   # 反应体系（连续加料）
    r = sys.add("NaOH", 0.1)                          # 自动反应，返回 Reaction
    r = chemkit.react({"NaOH": 0.1, "HCl": 0.1}, V=1.0)   # 一步式一次反应

    r.consumed, r.produced, r.pH, r.equation, r.equations   # 人类可读层
    r.consumed_raw, r.H_excess_raw                          # 引擎记账层
...
from .system import Reaction, System, react, default_tables, TABLES
